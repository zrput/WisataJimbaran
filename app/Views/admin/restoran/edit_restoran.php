...
<body>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Edit Data Restoran</h4>

                  <form class="forms-sample" action="<?= base_url('Restoran/update_restoran')?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="nama">Nama Restoran</label>
                      <input type="hidden" class="form-control" name="id" value="<?= $data->id_restoran?>">
                      <input type="text" class="form-control" name="nama" value="<?= $data->nama_restoran?>">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      <label for="jam">Jam Buka Restoran</label>
                      <input type="text" class="form-control" name="jam" value="<?= $data->jam_buka?>">
                    </div>
                    <div class="form-group">
                      <label for="harga">No Telepon Restoran</label>
                      <input type="number" class="form-control" name="telepon" value="<?= $data->telepon?>">
                    </div>
                    <div class="form-group">
                      <label for="peta">Link Peta</label>
                      <input type="text" class="form-control" name="peta" value="<?= $data->peta?>">
                    </div>
                    
                    <div class="form-group">
                      <label>Upload Gambar Restoran Baru</label>
                    ...
                    <label>Gambar Restoran Yang Sudah Dimasukkan</label>
                    <?php foreach ($img as $datas) : ?>
                    <div class="form-group" data-id="<?= $datas->id_gambar ?>">
                      <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" readonly  value="<?= $datas->gambar_restoran?>">
                        <span class="input-group-append">
                          <button class="btn btn-primary hapus-button" type="button" data-id="<?= $datas->id_gambar ?>">hapus</button>
                          <button class="btn btn-info lihat-button" type="button" data-gambar="<?= base_url('restoran/' . $datas->gambar_restoran) ?>">Lihat</button>
                        </span>
                      </div>
                    </div>
                    <?php endforeach;?>
  ...
  <script>
    ...
    function hapusData(id) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('Restoran/delete_img/') ?>' + id,
