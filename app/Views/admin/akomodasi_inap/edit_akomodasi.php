...
<body>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Edit Data Akomodasi Penginapan</h4>
                  <form class="forms-sample" action="<?= base_url('Akomodasi/update_akomodasi')?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="nama">Nama Penginapan</label>
                      <input type="hidden" class="form-control" name="id" value="<?= $data->id_penginapan?>">
                      <input type="text" class="form-control" name="nama" value="<?= $data->nama_penginapan?>">
                    </div>
                    <!-- radio button -->
                    <div class="form-group">
                        <label>Pilih Tipe Penginapan</label>
                        <div class="form-group">
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" value="Hotel" <?= ($data->tipe_penginapan == "Hotel")?"checked":""?>>
                              Hotel
                            </label>
                          </div>
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" Value="Villa" <?= ($data->tipe_penginapan == "Villa")?"checked":""?>>
                              Villa
                            </label>
                          </div>
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" value="Homestay" <?= ($data->tipe_penginapan == "Homestay")?"checked":""?>>
                              Homestay
                            </label>
                          </div>
                        </div>
                    ...
                    <div class="form-group">
                      <label for="alamat">Alamat Penginapan</label>
                      <input type="text" class="form-control" name="alamat" value="<?= $data->alamat?>">
                    </div>
                    <div class="form-group">
                      <label for="jam">Harga termurah dari fasilitas kamar</label>
                      <input type="number" class="form-control" name="min" value="<?= $data->min?>">
                    </div>
                    <div class="form-group">
                    <div class="row">
                      <div class="col-md-6">
                        <label for="checkIn">Waktu Check In Penginapan</label>
                        <input type="text" class="form-control" name="checkin" value="<?= $data->check_in ?>">
                      </div>
                      <div class="col-md-6">
                        <label for="checkOut">Waktu Check Out Penginapan</label>
                        <input type="text" class="form-control" name="checkout" value="<?= $data->check_out ?>">
                      </div>
                    </div>
                  </div>
                    <div class="form-group">
                      <label for="email">Email Penginapan</label>
                      <input type="email" class="form-control" name="email" value="<?= $data->email?>">
                    </div>
                    <div class="form-group">
                      <label for="nomor">Nomor Telepon Penginapan</label>
                      <input type="text" class="form-control" name="nomor" value="<?= $data->telepon?>">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      <label>Upload Gambar Penginapan Baru</label>
                    ...
                    <label>Gambar Penginapan Yang Sudah Dimasukkan</label>
                    <?php foreach ($img as $datas) : ?>
                    <div class="form-group" data-id="<?= $datas->id_gambar ?>">
                      <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" readonly  value="<?= $datas->gambar_akomodasi?>">
                        <span class="input-group-append">
                          <button class="btn btn-primary hapus-button" type="button" data-id="<?= $datas->id_gambar ?>">hapus</button>
                          <button class="btn btn-info lihat-button" type="button" data-gambar="<?= base_url('akomodasi_penginapan/' . $datas->gambar_akomodasi) ?>">Lihat</button>
                        </span>
                      </div>
                    </div>
                    <?php endforeach;?>
  ...
  <script>
    ...
    function hapusData(id) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('Akomodasi/delete_img/') ?>' + id,
