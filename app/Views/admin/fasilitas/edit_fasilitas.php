...
<body>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Edit Data Fasilitas Penginapan</h4>

                  <form class="forms-sample" action="<?= base_url('Fasilitas/update_fasilitas')?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="nama">Nama Fasilitas</label>
                      <input type="hidden" class="form-control" name="id_penginapan" value="<?= $data->id_penginapan?>">
                      <input type="hidden" class="form-control" name="id" value="<?= $data->id_fasilitas?>">
                      <input type="text" class="form-control" name="nama" value="<?= $data->nama_fasilitas?>">
                    </div>
                    <!-- radio button -->
                    <div class="form-group">
                        <label>Pilih Tipe Fasilitas</label>
                        <div class="form-group">
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" value="kamar" <?= ($data->jenis_fasilitas == "kamar")?"checked":""?>>
                              Kamar
                            </label>
                          </div>
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" Value="restoran" <?= ($data->jenis_fasilitas == "restoran")?"checked":""?>>
                              Restoran
                            </label>
                          </div>
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" value="kolam" <?= ($data->jenis_fasilitas == "kolam")?"checked":""?>>
                              Kolam Renang
                            </label>
                          </div>
                          <div class="form-check">
                            <label class="form-check-label">
                              <input type="radio" class="form-check-input" name="tipe" id="optionsRadios1" value="event" <?= ($data->jenis_fasilitas == "event")?"checked":""?>>
                              Aula Event
                            </label>
                          </div>
                        </div>
                    ...
                    <div class="form-group">
                      <label for="peta">Harga Fasilitas</label>
                      <input type="number" class="form-control" name="harga" value="<?= $data->harga_fasilitas?>">
                    </div>
                    
                    <div class="form-group">
                      <label>Upload Gambar Fasilitas Baru</label>
                    ...
                    <label>Gambar Fasilitas Yang Sudah Dimasukkan</label>
                    <?php foreach ($img as $datas) : ?>
                    <div class="form-group" data-id="<?= $datas->id_gambar ?>">
                      <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" readonly  value="<?= $datas->gambar_fasilitas?>">
                        <span class="input-group-append">
                          <button class="btn btn-primary hapus-button" type="button" data-id="<?= $datas->id_gambar ?>">hapus</button>
                          <button class="btn btn-info lihat-button" type="button" data-gambar="<?= base_url('fasilitas/' . $datas->gambar_fasilitas) ?>">Lihat</button>
                        </span>
                      </div>
                    </div>
                    <?php endforeach;?>
  ...
  <script>
    ...
    function hapusData(id) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('Fasilitas/delete_img/') ?>' + id,
