...
<body>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Form Edit Data Tempat Rekreasi Wisata</h4>
                  <p class="card-description">
                    Basic form elements
                  </p>
                  <form class="forms-sample" action="<?= base_url('Aktifitas_rekreasi/update_rekreasi')?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                      <label for="nama">Nama Tempat Rekreasi</label>
                      <input type="hidden" class="form-control" name="id" value="<?= $data->id_rekreasi?>">
                      <input type="text" class="form-control" name="nama" value="<?= $data->nama_tempat?>">
                    </div>
                    <div class="form-group">
                      <label for="alamat">Alamat Tempat Rekreasi</label>
                      <input type="text" class="form-control" name="alamat" value="<?= $data->alamat?>">
                    </div>
                    <div class="form-group">
                      <label for="jam">Jam Operasi Tempat Rekreasi</label>
                      <input type="text" class="form-control" name="jam" value="<?= $data->jam_operasi?>">
                    </div>
                    <div class="form-group">
                      <label for="exampleTextarea1">Harga Tiket Tempat Rekreasi</label>
                      <textarea class="form-control" id="exampleTextarea1" name="harga" rows="4"><?= $data->biaya_masuk?></textarea>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      <label>Upload Gambar Tempat Rekreasi Baru</label>
                    ...
                    <label>Gambar Tempat Rekreasi Yang Sudah Dimasukkan</label>
                    <?php foreach ($img as $datas) : ?>
                    <div class="form-group" data-id="<?= $datas->id_gambar ?>">
                      <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" readonly  value="<?= $datas->gambar_rekreasi?>">
                        <span class="input-group-append">
                          <button class="btn btn-primary hapus-button" type="button" data-id="<?= $datas->id_gambar ?>">hapus</button>
                          <button class="btn btn-info lihat-button" type="button" data-gambar="<?= base_url('aktifitas_rekreasi/' . $datas->gambar_rekreasi) ?>">Lihat</button>
                        </span>
                      </div>
                    </div>
                    <?php endforeach;?>
  ...
  <script>
    ...
    function hapusData(id) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('Aktifitas_rekreasi/delete_img/') ?>' + id,
